...
game-specific template cards with a selected template, character lists, optional
ohmesh-backed account storage.
...
- Record data stores custom game metadata plus game-keyed mission checks,
  vocabulary, story notes, sentence notes, game-specific template cards,
  selected template id, and character lists in the compact v3 shape documented
  in `README.md`.
...
- Built-in games should document their path, default data, and any game-specific
  ...
  defaults, including empty template card, selected template, and character
  lists.
