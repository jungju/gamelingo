...
6. 상단의 `플레이 미션 보기`를 누르면 선택한 게임에 맞는 설명 팝업이 열립니다.
7. 기본 UI에서는 `이번 플레이 미션`에서 이번 플레이 때 챙길 작은 목표를 체크합니다.
8. 게임 중 짧은 영어 문장을 발견하면 `문장 추가`에 원문을 그대로 입력합니다.
9. 상단의 `알아두어야 할 단어`에 자주 들리는 단어와 뜻을 저장합니다.
10. 오른쪽 `스토리 메모`에 장면, 인물 관계, 다음에 기억할 내용을 적습니다.
11. 게임 추가/수정의 `화면 템플릿`에서 기본 목록형 UI나 카드 팝업 UI를 게임별로 선택합니다.
12. 카드 팝업 UI에서는 미션 패널 없이 세로형 문장 카드를 많이 모아 보고, 카드를 누르면 큰 팝업으로 저장된 문장을 확인합니다.
13. `등장인물`에는 선택한 게임의 인물 이름과 짧은 설명을 추가합니다.
14. 필요하면 뜻과 내 문장을 가볍게 덧붙입니다.
15. 저장된 카드에서 `원문 듣기`와 `내 문장 듣기`를 눌러 소리 내서 연습합니다.
16. 익숙해진 문장은 `연습 완료`로 체크합니다.
...
- record data: 개인 게임 목록과 게임별 미션 체크, 단어장, 스토리 메모, 문장 노트, 화면 템플릿, 등장인물
...
상위 압축 키는 `v` 버전, `g` 커스텀 게임 목록, `n` 게임별 노트입니다. 노트 안의 압축 키는 `w` 단어 key:value 리스트, `m` 스토리 메모, `c` 미션 체크 bitmask, `s` 문장 배열입니다. 화면에서는 이 데이터를 다시 UI 구조로 복원해서 사용합니다.
화면 템플릿은 카드 팝업 UI를 쓸 때만 `u: "card-popup"`으로 저장하고, 기본 UI는 저장하지 않습니다. 등장인물은 `p`에 저장되며, 값이 없으면 저장하지 않습니다.
