...
export function HomePage({
  ...
  function deleteGame(game) {
    if (window.confirm(`Delete "${game.title}" and its saved board?`)) {
      ...
      <div className="mx-auto grid min-h-[calc(100vh-40px)] w-full max-w-6xl content-center gap-5">
        <header className="grid gap-5 rounded-[2rem] border border-stone-800 bg-stone-900/80 p-5 shadow-2xl md:grid-cols-[280px_1fr]">
          <div className="rounded-3xl border border-stone-800 bg-stone-950 p-5">
            <div className="mb-5 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-stone-500">Game language board</p>
              </div>
            </div>
            <AccountSummary
              syncState={syncState}
          ...
          <div className="flex flex-col justify-between gap-5">
            <div>
              <p className="mb-2 text-xs font-black uppercase tracking-wider text-stone-500">Game Library</p>
              <h1 className="max-w-2xl text-4xl font-black tracking-tight md:text-5xl">Choose today's game board</h1>
              <p className="mt-4 max-w-2xl leading-7 text-stone-400">Collect lines, vocabulary, and characters on one board as the story unfolds.</p>
            </div>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3" aria-label="Game library">
          {games.map((game) => (
            <article key={game.id} className="rounded-[2rem] border border-stone-800 bg-stone-900/70 p-4 shadow-xl">
              ...
              >
                ...
                <span className="grid content-center">
                  ...
                  <span className="mt-5 text-xs font-black uppercase tracking-wider text-stone-400">Open board</span>
                </span>
              </a>
              {game.isCustom ? (
...
function AccountSummary({ syncState, user, isGuestMode, onLogout, onSaveToOhmesh }) {
  return (
    <div className="grid gap-3 border-t border-stone-800 pt-4">
      <div>
        <p className="break-words text-sm font-black text-stone-100">{isGuestMode ? "Guest mode" : getUserDisplayName(user)}</p>
      ...
      >
        <form className="grid gap-4" onSubmit={saveGame}>
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="mb-1 text-xs font-black uppercase tracking-wider text-stone-500">Game Management</p>
              <h2 id="game-editor-title" className="text-2xl font-black">{isEditing ? "Edit game" : "Add game"}</h2>
            </div>
          ...
          <label className="grid gap-1.5">
            <span className="text-xs font-black text-stone-500">Title</span>
          ...
          <label className="grid gap-1.5">
            <span className="text-xs font-black text-stone-500">Description</span>
